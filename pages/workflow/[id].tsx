...
export default withPluginProtection(WorkflowEditor, PLUGIN_IDS.WORKFLOW);
...
function WorkflowEditor() {
  ...
  useEffect(() => {
    if (id) {
      const workflow = notes.find(note => note.id === id && note.type === 'workflow');
      if (workflow && 'tasks' in workflow) {
    ...
    <div className="flex flex-col h-[calc(100vh-80px)] md:h-screen p-5 animaMini">
      <div className="flex flex-col gap-4 mb-4">
        <div className="flex self-start flex-col justify-between items-start gap-2 md:items-center">
          <h1 className="font-semibold text-2xl">Edit workflow</h1>
        </div>
        <input
          type="text"
